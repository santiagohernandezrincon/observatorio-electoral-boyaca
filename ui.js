...
function abrirSidebar() {
    ...
    // Nuevo layout map-first: mostrar sección de stats en panel izquierdo
    const statsPanel = document.getElementById('obs-panel-stats');
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // ── View switching (nuevo layout map-first) ──
    document.querySelectorAll('.obs-view-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            const view = btn.dataset.view;
            document.querySelectorAll('.obs-view-btn').forEach(b => b.classList.remove('obs-view-btn--active'));
            btn.classList.add('obs-view-btn--active');
            document.querySelectorAll('.obs-vista').forEach(v => v.classList.remove('obs-vista--active'));
            const vista = document.getElementById('obs-vista-' + view);
            if (vista) vista.classList.add('obs-vista--active');
            // Invalidar mapa al volver a vista mapa
            if (view === 'mapa') {
                setTimeout(() => {
                    if (typeof mapSimple !== 'undefined' && mapSimple) mapSimple.invalidateSize();
                }, 80);
